/**
 * =============================================================================
 * VEFA: FRATERNAL & CIVIC COMMUNITY ENGINE (VERSION 2.4.0 - ENTERPRISE EDITION)
 * =============================================================================
 * File: app.js
 * Description: Core reactive engine featuring:
 *              - 1-Click Hall Rental Agreement & Deposit Receipt Generator
 *              - 1-Click Google Calendar & Apple iCal Sync Engine
...
const AppState = {
  ...
  hallDeposits: JSON.parse(localStorage.getItem('fraternal_hall_deposits')) || [
    { id: "dep-101", clientName: "Sarah Jenkins", eventDate: "2026-10-24", paymentMethod: "STRIPE", amount: 150.00, status: "Approved & Date Locked", timestamp: "2026-08-30T14:20:00Z" },
    { id: "dep-102", clientName: "Robert Hayes", eventDate: "2026-11-14", paymentMethod: "CASHAPP ($cashtag)", amount: 150.00, status: "Pending Review", timestamp: "2026-08-31T09:15:00Z" }
  ],
  rafflePot: 640.00,
  tvCurrentSlide: 0,
...
// =============================================================================
// 1-CLICK CALENDAR SYNC ENGINE (GOOGLE / APPLE ICAL)
// =============================================================================
function addToGoogleCalendar(title, dateStr, description, location) {
  sfx.playClick();
  const baseUrl = "https://calendar.google.com/calendar/render?action=TEMPLATE";
  const startIso = (dateStr ? dateStr.replace(/[^0-9]/g, '') : "20260901T190000Z");
  const endIso = (dateStr ? dateStr.replace(/[^0-9]/g, '') : "20260901T210000Z");
  const url = `${baseUrl}&text=${encodeURIComponent(title)}&dates=${startIso}/${endIso}&details=${encodeURIComponent(description)}&location=${encodeURIComponent(location || AppState.config.address)}`;
  window.open(url, '_blank');
  showToast(`📅 Opening Google Calendar for "${title}"...`, 'emerald');
}

function downloadICalEvent() {
  sfx.playClick();
  window.location.href = 'api/calendar.php';
  showToast('📅 Downloading Lodge Events .ics Calendar...', 'emerald');
}
...
function saveSetup() {
  sfx.playClick();
  const configString = `/**
 * VEFA: Fraternal & Civic Community Engine (v2.4.0)
  ...
  grid.innerHTML = AppState.tournamentGames.map(game => `
    <div class="tournament-card">
      ...
      <div style="display: flex; gap: 0.5rem; margin-top: auto; padding-top: 0.85rem; border-top: 1px solid var(--border-subtle);">
        ...
        <button class="btn-secondary" style="font-size: 0.88rem;" onclick="addToGoogleCalendar('${escapeHTML(game.name)}', '20260903T183000Z', '${escapeHTML(game.format)} with GM ${escapeHTML(game.gameMaster)}', 'Lodge Social Quarters')">
          📅 Add Cal
        </button>
      </div>
    </div>
...
// =============================================================================
// MODULE 7: 1-CLICK HALL RENTAL AGREEMENT & DEPOSIT RECEIPT GENERATOR
// =============================================================================
function generateRentalAgreementPDF(depositId) {
  sfx.playClick();
  const dep = AppState.hallDeposits.find(d => d.id === depositId) || AppState.hallDeposits[0] || {
    id: "dep-sample",
    clientName: "Jane Doe",
  ...
  if (printWindow) {
    printWindow.document.write(`
      ...
      <html>
      <head>
        <title>Hall Rental Agreement & Deposit Receipt - ${escapeHTML(dep.clientName)}</title>
        <style>
          body { font-family: 'Times New Roman', serif; padding: 2.5rem; color: #0f172a; max-width: 800px; margin: 0 auto; line-height: 1.5; }
          .header { text-align: center; border-bottom: 2px solid #d4af37; padding-bottom: 1rem; margin-bottom: 1.5rem; }
          .receipt-box { border: 1px solid #cbd5e1; background: #f8fafc; padding: 1rem 1.5rem; margin: 1.5rem 0; border-radius: 4px; }
          .terms { font-size: 0.88rem; text-align: justify; margin-bottom: 2rem; }
          .signatures { display: grid; grid-template-columns: 1fr 1fr; gap: 3rem; margin-top: 3rem; }
          .sig-line { border-top: 1px solid #000; padding-top: 0.4rem; font-size: 0.85rem; }
        </style>
      </head>
      <body>
        <div class="header">
          <h2 style="margin:0; font-size: 1.6rem; text-transform: uppercase;">${escapeHTML(AppState.config.organizationName)} ${escapeHTML(AppState.config.chapterNumber)}</h2>
          <p style="margin: 0.2rem 0; font-size: 0.9rem;">${escapeHTML(AppState.config.address)} • Phone: ${escapeHTML(AppState.config.phoneMain)}</p>
          <h3 style="margin-top: 0.6rem; color: #1e3a8a;">HALL RENTAL AGREEMENT & DEPOSIT CONFIRMATION</h3>
        </div>

        <div class="receipt-box">
          <table style="width: 100%; border-collapse: collapse; font-size: 0.95rem;">
            <tr><td><strong>Agreement / Deposit ID:</strong></td><td>${escapeHTML(dep.id)}</td></tr>
            <tr><td><strong>Renter / Client Name:</strong></td><td>${escapeHTML(dep.clientName)}</td></tr>
            <tr><td><strong>Reserved Event Date:</strong></td><td>${escapeHTML(dep.eventDate)}</td></tr>
            <tr><td><strong>Payment Method:</strong></td><td>${escapeHTML(dep.paymentMethod)}</td></tr>
            <tr><td><strong>Deposit Amount Paid:</strong></td><td><strong>$${(dep.amount || 150.00).toFixed(2)} USD (100% Refundable)</strong></td></tr>
            <tr><td><strong>Reservation Status:</strong></td><td><strong style="color: #059669;">${escapeHTML(dep.status)}</strong></td></tr>
          </table>
        </div>

        <div class="terms">
          <h4 style="margin-bottom: 0.4rem;">TERMS AND CONDITIONS OF VENUE RENTAL</h4>
          <ol style="padding-left: 1.2rem;">
            <li><strong>Damage & Cleanliness Deposit:</strong> The $150.00 refundable deposit confirms the date reservation and guarantees adherence to facility rules. Full refund will be disbursed within 5 business days following post-event inspection by the Lodge Hall Steward.</li>
            <li><strong>Occupancy Limits:</strong> The Grand Ballroom capacity is capped at ${AppState.config.hallRental.ballroomCapacity || 150} seated guests. Kitchen access is limited to authorized catering staff.</li>
            <li><strong>Alcohol Policy:</strong> All alcoholic beverages must be served strictly in accordance with State Liquor Control regulations. No outside alcohol permitted on premises without prior written permit.</li>
            <li><strong>Closing Time:</strong> All music and bar service must conclude by 11:30 PM, with premises vacated by midnight.</li>
          </ol>
        </div>

        <div class="signatures">
          <div>
            <div class="sig-line">Renter Signature / Date</div>
          </div>
          <div>
            <div class="sig-line">Lodge Steward / Trustee Signature / Date</div>
          </div>
        </div>
      </body>
      </html>
    `);
    printWindow.document.close();
    showToast(`📄 Rental Agreement generated for ${dep.clientName}!`, 'emerald');
  }
}
...
function renderDepositLedger() {
  ...
  tbody.innerHTML = AppState.hallDeposits.map(dep => `
    <tr>
      ...
      <td>
        <select class="form-control" style="padding: 0.25rem 0.5rem; font-size: 0.8rem; min-height: 36px; display: inline-block; width: auto;" onchange="updateDepositStatus('${escapeHTML(dep.id)}', this.value)">
        ...
        <button class="btn-secondary" style="padding: 0.2rem 0.5rem; font-size: 0.76rem; margin-left: 0.3rem;" onclick="generateRentalAgreementPDF('${escapeHTML(dep.id)}')">
          🖨️ Contract
        </button>
      </td>
    </tr>
...
// =============================================================================
// MODULE 8: MULTI-PAYMENT HUB FOR $150 HALL DEPOSIT
// =============================================================================
...
async function handleHallDepositSubmit(event) {
  ...
  // Prompt user to print agreement
  setTimeout(() => {
    if (confirm("Deposit confirmed! Would you like to view and print your Hall Rental Agreement & Receipt now?")) {
      generateRentalAgreementPDF(depositRecord.id);
    }
  }, 400);
}
...
const DEVOPS_CODE_FILES = {
  ...
  'openapi.json': `{\n  "openapi": "3.1.0",\n  "info": { "title": "VEFA Platform API", "version": "2.4.0" }\n}`
};
...
async function runDevOpsApiTest() {
  ...
  setTimeout(() => {
    let mockResponse = {};
    if (endpoint === 'health') {
      mockResponse = { status: "healthy", version: "2.4.0", db_driver: "mysql_or_sqlite", php_version: "8.2.14", twilio_configured: true };
...
document.addEventListener('DOMContentLoaded', () => {
  document.title = `${AppState.config.organizationName} ${AppState.config.chapterNumber} | VEFA Platform v2.4.0`;
